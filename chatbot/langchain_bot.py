...
def handle_exception(e):
    ...
    return jsonify({"text": "An internal error occurred.", "audio_base64": None}), 500
...
def delete_chat():
    ...
    return jsonify({"status": "error", "message": "Chat not found"}), 400
...
def chat():
    ...
    if not user_input:
        return jsonify({"text": "Please enter a message.", "audio_base64": None}), 400

    if 'chats' not in session or not session.get('current_chat'):
        return jsonify({"text": "Please create or select a chat first.", "audio_base64": None}), 400

    current_chat = next((chat for chat in session['chats'] if chat['id'] == session['current_chat']), None)
    if not current_chat:
        return jsonify({"text": "Chat not found.", "audio_base64": None}), 404
    ...
    try:
        ...
        return jsonify({"text": "An unexpected error occurred.", "audio_base64": None}), 500
